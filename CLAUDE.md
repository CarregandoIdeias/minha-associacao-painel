...
## Portal do associado — mini-dashboard "Início", ficha completa, aba Financeiro, logo da associação (28/07/2026)

Vários pedidos do usuário no mesmo dia, todos só em `portal.html` (exceto a logo, que é em `index.html`) — sem mudança de schema.

- **Logo da associação** (`index.html`, seção `#secao-parametrizacao`): card novo "Logo da associação", mesmo padrão de upload de `salvarFotoAssociado()` do `portal.html` (`FileReader` + `<canvas>` redimensionando pra 300px + `toDataURL('image/jpeg', 0.85)`), consumindo `PUT /configuracoes/logo` (novo, `admin` only — ver `backend/CLAUDE.md`). Atualiza `estado.logoAssociacao` e reflete na hora no header do Dashboard, que já lia esse valor desde 27/07.
- **Mini-dashboard "Início"** (`portal.html`, novo item de sidebar, primeiro da lista, landing page do login): dois cards lado a lado (`.resumo-grid`) — "Situação financeira" (badge de status, próxima cobrança pendente com botão "Pagar com Pix" reaproveitando `abrirModalPix()`, contadores de pendentes/atrasadas) e "Comunicados" (badge de não lidos, 3 mais recentes, botão "Ver todos"). Só reaproveita rotas já existentes (`GET /portal/meus-dados`, `GET /portal/minhas-cobrancas`, `GET /comunicados`), nenhuma nova.
  - **Detalhe não-óbvio**: `abrirModalPix()` depende de `minhasCobrancasCache` (variável global), que antes só era populada por `renderizarMinhasCobrancas()` — chamada só ao abrir "Meus Dados"/"Financeiro". Se o associado clicasse "Pagar com Pix" direto do card de Início sem nunca ter aberto essas abas, o cache estaria vazio e o botão não faria nada silenciosamente. Corrigido populando o mesmo cache em `renderizarResumoFinanceiro()` também. Ao reaproveitar `abrirModalPix()`/`minhasCobrancasCache` em qualquer tela nova, lembrar dessa dependência.
  - **Gap real corrigido de quebra**: `POST /comunicados/:id/marcar-lido` (rota do backend desde a Fase 3/item de sprint 3) nunca tinha consumidor aqui — abrir o mural nunca marcava nada como lido de verdade, então o badge de não lidos do Início nunca zeraria. `marcarComunicadosVisiveisComoLidos()`, chamada ao final de `carregarComunicados()`, resolve isso (fire-and-forget, não bloqueia a renderização).
- **Header**: `#app-header-email` (id mantido, conteúdo repropriado) agora mostra o **nome da associação** (`GET /configuracoes/identidade`, liberado pra qualquer papel do tenant, não só admin/diretoria) em vez do e-mail do associado. `.app-header-avatar-btn .nome` ganhou `color: var(--text)` explícito — mesmo bug de cor preta ilegível no tema escuro já documentado e corrigido em `index.html` (seção "Menu de perfil reorganizado" acima), só que esse arquivo tinha ficado de fora da correção na época.
- **"Meus Dados" virou ficha completa**: `perfil-associado-container` trocou o resumo de 4 linhas por 3 seções (`.ficha-secao`/`.ficha-grid`/`.ficha-item`, CSS novo só deste arquivo — `index.html` tem um padrão parecido mas com nomes de classe diferentes, não são a mesma classe) — Dados pessoais (nome, CPF, RG, telefone, e-mail), Endereço, Plano e situação (categoria, badge de status, associado desde, cadastro em). Só leitura, sem edição (o associado não edita a própria ficha, só a foto). `GET /portal/meus-dados` ganhou os campos novos pro backend (ver `backend/CLAUDE.md`) — **e-mail vem de `estado.email`** (já decodificado do JWT no login), não do backend, porque `associados` não tem coluna de e-mail própria.
- **Nova aba "Financeiro"**: a tabela "Minhas cobranças" saiu de dentro de "Meus Dados" e virou seção própria (`#secao-financeiro`, `carregarFinanceiro()`) — mesmo container/render (`tabela-minhas-cobrancas-container`/`renderizarMinhasCobrancas()`), só o ponto de entrada mudou.
- **Carteirinha digital** (mesmo dia, pedido separado): botão "Ver carteirinha" ao lado de "Trocar foto" abre `#overlay-modal-carteirinha` — um cartão (`.carteirinha-card`) com foto, nome, associação, categoria e status, gerado na hora a partir de `associadoAtual` (variável nova, cache do último `GET /portal/meus-dados` carregado por `carregarMeusDados()` — nenhuma rota nova). QR code reaproveita a lib `qrcode-generator`/`renderizarQrCode()` já usada pro Pix, codificando só `ASSOCIADO:<id>` — **é cosmético/identificador, não tem endpoint de verificação por scan**. Se a diretoria pedir controle de acesso de verdade em evento (portaria escaneando pra validar situação), isso vira uma rota pública nova que recebe o id e devolve só ativo/inadimplente, sem dado pessoal — não implementado ainda, decisão consciente de escopo.
